...
export class PrivateWorkDetail {
  ...
  readonly materialColumns = ['materialName', 'vendorName', 'amount', 'paymentDate', 'actions'];
  ...
  addMilestone(): void {
    ...
    const ref = this.dialog.open<MilestoneForm, MilestoneFormDialogData>(MilestoneForm, {
      data: { milestone: null, nextSortOrder }
    });
    ref.afterClosed().subscribe((result) => {
      if (!result) return;
  ...
  editMilestone(milestone: PrivateWorkMilestone): void {
    const ref = this.dialog.open<MilestoneForm, MilestoneFormDialogData>(MilestoneForm, {
      data: { milestone, nextSortOrder: milestone.sortOrder }
    });
    ref.afterClosed().subscribe((result) => {
      if (!result) return;
